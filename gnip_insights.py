...
class Engagement(Insights):
	...
	engagement_28hr_historical_json_encoded = json.loads('{ "engagement_types": ["impressions", "engagements", "favorites", "replies", "retweets", "url_clicks", "hashtag_clicks", "detail_expands", "permalink_clicks", "media_clicks", "app_install_attempts", "user_follows", "user_profile_clicks", "video_views" ]}')
...
class Audience(Insights):
	...
	def get_segment(self, id):
		...
		get_request = requests.get(Insights.base_url + "audience/segments/" + id
				, auth = Insights.get_auth(self)
                , headers = Insights.json_header
                )
		return get_request	
